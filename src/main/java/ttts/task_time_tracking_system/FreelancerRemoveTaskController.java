...
public class FreelancerRemoveTaskController implements Initializable {
    @FXML
    ...
    @FXML
    void removeTask(ActionEvent event) throws IOException, ClassNotFoundException{
        ...
        for (Tasks t : tasks.values()) {
            if (t.getIdTask() == actualTask.getIdTask()) {
                tasks.remove(actualTask.getIdTask());
        ...
        try {
            for (Tasks t : RepositoryTasks.getRepositoryTasks().getTasks().values()) {
                if(SessionData.freelancer.getNIF().equals(t.getFreelancer().getNIF())) {
                    if(t.getState().equals(TaskState.EM_CURSO)||t.getState().equals(TaskState.FINALIZADO)) {
